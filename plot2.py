...
def subtractLists(list1, list2):
    ...
    if(len(list1) == len(list2)):
        ...
        while temp < len(list1):
            new_list.append((list1[temp] - list2[temp])*(a_value[temp]-z_value[temp]))
            temp+=1
    return new_list
...
def findIndex(n):
    ...
    while temp < len(a_value):
        print "hey"
        if(a_value[temp] - z_value[temp] == n):
            return temp
        temp += 1
...
ReadFile()
index28 = findIndex(28)
index50 = findIndex(50)
index82 = findIndex(82)
index126 = findIndex(126)

import matplotlib.pyplot as plt
subtracted_list = subtractLists(binding_per_nuc, liquidDropEnergies())
plot1 = plt.plot(z_value, subtracted_list , 'bo')
plot1 = plt.plot([z_value[index28],z_value[index50],z_value[index82],z_value[index126]] ,[subtracted_list[index28],subtracted_list[index50],subtracted_list[index82],subtracted_list[index126]] , 'ro')
